...
columns = ['Down', 'Distance', 'Yards From Own Goal', 'Total Seconds Left', 'Score Difference', 'Play Type']
df = pd.DataFrame(full_season_data, columns=columns)

print("--- Data Loaded ---")
print(f"Total plays in the dataset: {len(df)}")
print(df.head())
print("\n")
...
features = df.drop('Play Type', axis=1)
...
seed = 42
np.random.seed(seed)  # Updated to use NumPy's random seed
...
# Define the optimizer. SPSA is a good gradient-free optimizer suitable for
# the noisy and complex loss landscapes often found in quantum machine learning.
optimizer = SPSA(max_iterations=100)

# Set up the quantum instance. We will use the Sampler for circuit execution.
...
vqc = VQC(
    ...
    ansatz=ansatz,
    optimizer=optimizer,
    sampler=quantum_instance  # Updated parameter name
)
